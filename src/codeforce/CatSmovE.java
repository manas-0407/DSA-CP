...
public class CatSmovE {
    ...
    private static void manas()throws IOException {
        ...
        for(i=0;i<k;i++){
            int xx=sc.nextInt();
            if(!tm.containsKey(xx)) tm.put(xx,1);
            else tm.put(xx,tm.get(xx)+1);
        }
        List<Pair> al=new ArrayList<>(tm.size());
        i=tm.size()-1;
        for(Map.Entry<Integer,Integer> en:tm.entrySet()){
            al.add(new Pair(en.getKey(),en.getValue()));
        }
        int ct=0,mouse=0;
        for(;i>=0;i--){
            if(al.get(i).key > ct){
                for(int j=0;j<al.get(i).val;j++){
                    if(al.get(i).key > ct) mouse++;
                    else break;
                    ct+=n-al.get(i).key;
                }
            } else break;
