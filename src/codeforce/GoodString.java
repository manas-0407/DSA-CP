...
public class GoodString {
    ...
    public static void main(String[] args)throws IOException {
        ...
        while (t-- > 0){
            ...
            int i,n=s.length();
            int[][] dp=new int[n][10]; // mapping of index to the subsequence of digits;
            HashMap<Integer,Integer> last_vist = new HashMap<>();
//            Arrays.fill(dp[0],1);
            for(int[] a:dp) Arrays.fill(a,0);
//            last_vist.put(s.charAt(0)-48,0);
            for(i=0;i<n;i++){
                for(int j=0;j<10;j++){
                    if(s.charAt(i)-48 == j){dp[i][j] = 1+(last_vist.getOrDefault(j,-1)!=-1?dp[last_vist.get(j)][s.charAt(i)-48]:0);
                    }else
                    dp[i][j] = 1+(last_vist.getOrDefault(j,-1)!=-1?(dp[last_vist.get(j)][s.charAt(i)-48]==0?dp[last_vist.get(j)][s.charAt(i)-48]+1:dp[last_vist.get(j)][s.charAt(i)-48]):-1);
                }
                last_vist.put(s.charAt(i)-48,i);
            }
            int max=1;
            for(i=0;i<n;i++){
                for(int j=0;j<10;j++){
                    if(s.charAt(i)-48 == j){
                        ...
                        if(dp[i][j] % 2 == 0) max=Math.max(max,dp[i][j]);
                    }
                }
            }
            out.println(n-max);
