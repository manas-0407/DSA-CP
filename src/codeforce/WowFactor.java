...
public class WowFactor {
    ...
    public static void main(String[] args)throws IOException {
            ...
            // vvooovovooovvooovvovo

            for(i=0;i<n;) {
                char c=s.charAt(i);
                if(c=='v'){
                    while(i<n && s.charAt(i)==c) {i++;vc++;}
                    if(vc>1) al.add(vc-1);
                    vc=0;
                }
                else {
                    while (i<n && !(i<n-1 && s.charAt(i)==s.charAt(i+1) && s.charAt(i)=='v')) {if(s.charAt(i)=='o')oc++;i++;}
                    if(al.size()>0) al.add(oc);
                    oc=0;
                }
            }
            if(al.size()==0) {
            ...
            long ans=0,sum=0;
            wtill=al.get(0);
            for(i=2;i<al.size();i+=2){
                sum+= (long) al.get(i - 1) * wtill;
                ans+=al.get(i)*sum;
                wtill+=al.get(i);
            }
            out.println(ans);
