...
//        while (t-- > 0){
            ...
            while (n-- > 0){
                ...
                maxi=Math.min(maxi,b);
            }
            debug(mini);
            debug(maxi);
            if(maxi<mini) out.println(-1);
            else if(x0>=mini && x0<=maxi) out.println(0);
            else {
                out.println(Math.abs(mini-x0) < Math.abs(maxi-x0) ? Math.abs(mini-x0) : Math.abs(maxi-x0));
            }
            out.flush();
//        }
    }
