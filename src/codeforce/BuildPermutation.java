...
public class BuildPermutation {
    ...
    static void recbuild(int n,int[] sqTable){

            if(n==0) return;
            if(n==1) {System.out.print(0+" "); return;}
            if(n==2) {
                System.out.print(1+" "+0+" "); return;
            }
            int y=2*(n-1);
            int l=0,r=sqTable.length-1,mid,nextsq=0;
            while(l<=r){
                 mid = l+(r-l)/2;
            ...
            debug(nextsq);
            recbuild(nextsq-n+1,sqTable);
            for(int i=n-1;i>=nextsq-n+1;i--){
                System.out.print(i+" ");
            }
    }
    ...
    public static void main(String[] args)throws IOException {
        ...
        int[] squareTable=new int[448];
        for(int i=0;i<448;i++)
            squareTable[i]=i*i;
        int t=sc.nextInt();
        while(t-- > 0){
            int n=sc.nextInt();
            recbuild(n,squareTable);
            System.out.println();
        }
    }
}
