...
public class AddOneTabVersion {
    ...
    public static void main(String[] args)throws IOException {
        long[] rai=new long[(int) (2*1e5+5)]; // each element is 10 and its index marks m operation
        manas: for(int i=0;i<2*1e5+5;i++){
            if(i==9) {rai[9]=3;continue manas;}
            rai[i] = i<9?2:(rai[i-9]%mod+rai[i-10]%mod)%mod;
        }
        int t=sc.nextInt();
        while(t-- > 0){
            int n=sc.nextInt();
            ...
            while(n!=0){
                int xx=n%10;
                sum+=m-(10-xx)>=0?rai[m-10+xx]:1%mod;
                n/=10;
            }
            out.println(sum%mod);
            out.flush();
