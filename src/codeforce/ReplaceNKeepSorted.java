...
public class ReplaceNKeepSorted {
    ...
    public static void main(String[] args)throws IOException {
        ...
        long[] pre = new long[n];
        for(i=0;i<n;i++)  pre[i] = i==0||i==n-1?(i==0?(a[i]-1):(k-a[i])):(pre[i-1]+a[i]-a[i-1]+a[i+1]-a[i]-2);
        while (q-->0){
            int l=sc.nextInt();
            int r=sc.nextInt();
//            long sum=a[l-1]-1 + a[l] - a[l-1] -1 + k-a[r-1]+a[r-1]-(r-2>=0?a[r-1]-a[r-2]-1:0);  //ele at l-1 and r-1    //1+3+1+4-1
            l--;r--;
            long sum=((a[l]-1) + (r>l?a[l+1]-a[l]-1:0)) + ((k-a[r]) + ((r>l)?a[r]-a[r-1]-1:0));
            l++;
            r--;
            if(l<=r){
                sum+=pre[r] - pre[l-1];
