...
public class MakeItRound {
    ...
    static long gcd(long a, long b) { // TC- O(logmax(a,b))
            if (b == 0) return a;
            return gcd(b, a % b);
    }
    ...
    static int digitCt(long x){
        if(x<10) return 1;
        return 1+digitCt(x/10);
    }
    static long ans(long n,long m,long tens){
        while (tens>0) {
            debug(tens);
            long ansss = lcm(n, tens);
            if(ansss <= n*m) return (ansss*((n*m)/(ansss)));
            else tens/=10;
        }
        return n*m;
    }
