...
public class Postcard {
    ...
    public static void main(String[] args)throws IOException {
        ...
        String s=sc.next();
        int k=sc.nextInt();
        int qct=0,sct=0,ct=0;
        int i;
        for(i=0;i<s.length();i++){
            if(s.charAt(i) == '?') qct++;
            else if(s.charAt(i)=='*') sct++;
            else ct++;
        }
//        if(k==ct){
//            for(i=0;i<s.length();i++){
//                if(s.charAt(i) != '?' && s.charAt(i) != '*') System.out.print(s.charAt(i));
//            }
//        }
        StringBuilder stt=new StringBuilder();
        if(k>=ct){
            if(sct == 0 && k>ct) System.out.println("Impossible");
            else {
                for(i=0;i<s.length();i++){
                    if(s.charAt(i) == '*'){
                        int vv=k-ct;
                        for(int zz=0;zz<vv;zz++) {ct++;stt.append(s.charAt(i-1));}
                    }
                    else if(s.charAt(i) != '?' && s.charAt(i) != '*') stt.append(s.charAt(i));
                }
                System.out.println(stt);
            }
        }
        else {
            if(sct+qct < ct-k) {System.out.println("Impossible");}
            else {
                int x=ct-k;
                for(i=0;i<s.length();i++){
                    if(s.charAt(i) != '?' && s.charAt(i) != '*'){
                        if(i<s.length()-1 && (s.charAt(i+1)=='?' || s.charAt(i+1)=='*') && x>0){
                            x--;
                        }
                        else stt.append(s.charAt(i));
                    }
                }
                System.out.println(stt);
            }
