...
public class DoubleSort {
    ...
    public static void main(String[] args)throws IOException {
        ...
        while(t-- > 0){
            ...
            Pair[] pp=new Pair[n];
            int i;
            for(i=0;i<n;i++) pp[i]=new Pair(sc.nextInt(),0);
            for(i=0;i<n;i++) pp[i].val=sc.nextInt();
            Pair[] pp1=pp.clone();
            HashMap<Integer,Integer> hss=new HashMap<>();
            for(i=0;i<n;i++){
                Pair xyz=new Pair(pp[i].key,pp[i].val);
                hss.put(xyz.hashCode(),i);
            }


            Arrays.sort(pp, (x,y) -> ((x.key+x.val) - (y.key+y.val)));
//            for(Pair pf: pp) System.out.println(pf.key+" "+pf.val);

            boolean b=true;
            for(i=1;i<n;i++){
                if((pp[i-1].key < pp[i].key && pp[i-1].val > pp[i].val) || ((pp[i-1].key > pp[i].key && pp[i-1].val < pp[i].val))) b=false;
            }
            if(b){
                int count=0;
                StringBuilder stb=new StringBuilder();
                while(n>1 && pp[n-1].key == pp[n-2].key && pp[n-1].val == pp[n-2].val) n--;
                for(i=0;i<n-1;i++){
                    if(hss.get(pp[i].hashCode()) != i) {
                        count++;
                        stb.append(1+hss.get(pp[i].hashCode())).append(" ").append(i+1).append("\n");
                        hss.put(pp1[i].hashCode(), hss.get(pp[i].hashCode()));
                        hss.put(pp[i].hashCode(), i);
                    }
                }
