...
public class AddOne {
        ...
        static long retVal(int x,int m,long[] dp){
            if(m<1) return 1;
            ...
            if(m<0) return 1;
            if(m==0) return 2;
            else return dp[mv]=(retVal(0,m+1,dp)%mod+retVal(0,m,dp)%mod)%mod;
        }
        ...
        static void manas() throws IOException {
            ...
            while(n!=0){
                int xx=n%10;
                sum+=(retVal(0,m+xx,dp)%mod);
                n/=10;
            }
            out.println(sum);
            out.flush();
        }
        static final int lll= (int) (2*1e5+5);
        static long[] dp=new long[lll];
        public static void main(String[] args)throws IOException {
            int t=sc.nextInt();
            Arrays.fill(dp,-1); //considering dp only accepts 0 and m
            while(t-- > 0) manas();
            out.close();
        }
