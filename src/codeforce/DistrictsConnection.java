...
public class DistrictsConnection {
    ...
    public static void main(String[] args)throws IOException {
        ...
        while (t-- > 0){
            ...
            Pair[] a=new Pair[n];
            int i;
            for(i=0;i<n;i++) {a[i]=new Pair(sc.nextInt(),i+1);}
            Arrays.sort(a, Comparator.comparingInt(x -> x.key));
            ...
            int ct=1;
            i=0;
            while (a[i].key == a[0].key && ct<n){
                out.println(a[i].val+" "+a[n-1].val);
                ct++;i++;
            }
            int j=n-2;
            while (ct<n){
                out.println(a[0].val+" "+a[j].val);
                j--;
                ct++;
            }}
            out.flush();
