...
public class EvenSubArray {
    ...
    static void manas(ArrayList<Integer> perf) throws IOException {
        int n=sc.nextInt();
        int i;
        int[] a=new int[n];
        ...
        int p=0;
        int count=0;
        while(perf.get(p) < (n<<1)){
            HashMap<Integer,Integer> hm=new HashMap<>();
            int persum=0;
            hm.put(persum,1);
            for(i=0;i<n;i++){
                persum^=a[i];
                int nownow=persum^ perf.get(p);
                if(hm.containsKey(nownow)) {count+=hm.get(nownow);}
                if(!hm.containsKey(persum)) hm.put(persum,1);
                else hm.put(persum,hm.get(persum)+1);
            }
            p++;
        }
        long tot=((long) n *(n+1))/2L;
        out.println(tot-count);
        out.flush();
    }
    public static void main(String[] args)throws IOException {
        ArrayList<Integer> perf=new ArrayList<>();
        perf.add(0);
        for(int i=1;i*i<=4*1e5;i++) perf.add(i*i);
        int t=sc.nextInt();
        while(t-- > 0){
            manas(perf);
        } out.close();
