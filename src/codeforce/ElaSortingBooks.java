...
public class ElaSortingBooks {
    ...
    private static void manas(FastRead sc,FastWriter out)throws IOException{
        ...
        while(t-- > 0){
            ...
            while(i<k){
                debug("Enter");
                int counter=n/k,minhold=Integer.MAX_VALUE,ptr=0,runtill=0;
                char ansholder='a';
                while(counter>0){
                    counter--;
                    if(tab[ptr] == 0) {runtill=ptr; ansholder=(char)('a'+ptr); break;}
                    minhold = Math.min(minhold, tab[ptr]);
                    ptr++;
                }
                if(minhold == Integer.MAX_VALUE && ptr==0) {
                    minhold=k-i;
                }
                debug(minhold);
                if(counter == 0) {runtill=ptr;ansholder=(char)('a'+ptr); }
                for(int j=0;j<runtill;j++){
                    tab[j] -= minhold;
                }
                int j;
                for(j=0;j<minhold && i<k &&minhold!=Integer.MAX_VALUE;j++) {i++;ss.append(ansholder);}
                debug(ss);
            }
