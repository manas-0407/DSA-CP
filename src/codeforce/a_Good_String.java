...
public class a_Good_String {
    ...
    static int goodStr(String s,int l,int r,char current_good){


//        debug(l+" "+r);
        if(l==r){
            return s.charAt(l)==current_good?0:1;
        }
        int mid=l + (r-l)/2; // inclusive mid
        int i,c1=0,c2=0;
        for(i=l;i<=mid;i++){
            if(s.charAt(i) != current_good) c1++;
        }
        for(i=mid+1;i<=r;i++){
            if(s.charAt(i) != current_good) c2++;
        }
        return Math.min(c1+goodStr(s,mid+1,r, (char) (current_good+1)) , c2+goodStr(s,l,mid, (char) (current_good+1)));
    }
    public static void main(String[] args)throws IOException {
        int t=sc.nextInt();while (t-- > 0){
            int n=sc.nextInt();
            String s=sc.nextString();
            out.println(goodStr(s,0,n-1,'a'));
            out.flush();
        }
    }
}
