...
public class NinjaJump2 {
    ...
    static int ninja(int[][] a,int[][] dp,int index,int day){

        if(day<0) return 0;
        if(dp[day][index] != -1) return dp[day][index];
        int max=Integer.MIN_VALUE;
        for(int i=0;i<3;i++){
            if(i!=index) max=Math.max(max,ninja(a,dp,i,day-1));
        }
        return dp[day][index]=max+a[day][index];
    } // this function returns best score if I pick element at given index.
    public static void main(String[] args)throws IOException {
        ...
        int[][] dp=new int[n][3];
        for(int[] pg:dp) Arrays.fill(pg,-1);
        int maxx=Integer.MIN_VALUE;
        for(int i=0;i<3;i++){
            maxx=Math.max(ninja(a,dp,i,n-1),maxx);
        }
        out.println(maxx);
        out.flush();
    }
