...
/*
This code works if there is there is unit pricing system
 */
...
public class BookShop {
    ...
    public static void main(String[] args)throws IOException {
        ...
        int i;
        int[] track=new int[n];
        for(i=0;i<n;i++) track[i]=sc.nextInt();
        for(i=0;i<n;i++){

            book.put(track[i],sc.nextInt());
        }
        long max;
        long[] dp=new long[x+1];
        long[] dptrack = new long[x+1];
        for(Map.Entry<Integer,Integer> en:book.entrySet()){
            for(i=1;i<=x;i++){
                max=(i>=en.getKey() ? en.getValue()+dp[i-en.getKey()] : 0);
                dptrack[i]=max;
            }
