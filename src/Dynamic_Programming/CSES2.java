...
public class CSES2 {
    ...
    static int minCoin(int sum,int[] coins,int[] dp){

        if(sum<0) return -1;
        if(sum==0) return dp[0]=0;
        if(dp[sum]!=-1) return dp[sum];
        int min=Integer.MAX_VALUE;
        for (int coin : coins) {
            int getVal = minCoin(sum - coin, coins,dp);
            if(getVal != -1) min = Math.min(min, getVal+1);
        }
