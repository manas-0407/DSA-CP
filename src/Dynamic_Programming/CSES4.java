...
public class CSES4 {
    ...
    static long getDist(int[] coins, int x, int index,long[][] dp) {

        if (x == 0) return dp[x][index]=1;
        if (x < coins[0]) return 0;
        if(dp[x][index] != -1) return dp[x][index];
        long val = 0;
        for (int i = 0; i < coins.length; i++) {
            if (x >= coins[i] && i >= index) val += getDist(coins, x - coins[i], i,dp);
        }
        return dp[x][index]=val%mod;
    }
