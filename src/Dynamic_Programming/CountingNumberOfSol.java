...
public class CountingNumberOfSol {
    ...
    static int cointake(int[] dp,int[] coin,int sum){
            if(sum<0) return 0;
            if(sum==0) return  dp[0]=1;
            if(dp[sum] != -1)return dp[sum];
            int ways=0;
        for (int j : coin) {
            if (sum - j >= 0) {
                ways += cointake(dp, coin, sum - j);
            }
        }
            return dp[sum]= ways;
    }
    ...
    public static void main(String[] args)throws IOException {
        ...
        int coin=sc.nextInt();
        int[] c=new int[coin];
        for(int i=0;i<coin;i++) c[i] =sc.nextInt();
        int sum=sc.nextInt();
        int[] dp=new int[sum+1];
        Arrays.fill(dp,-1);
        System.out.println(cointake(dp,c,sum));
    }
}
