...
public class CSES9Tab {
    ...
    public static void main(String[] args)throws IOException {
        long[][] dp=new long[(int) (1e6+1)][2];
        dp[1][0]=1;
        dp[1][1]=1;
        for(int i=2;i<(1e6+1);i++){
            dp[i][0]=dp[i-1][0]*4 + dp[i-1][1];
            dp[i][0]=dp[i][0]>mod?dp[i][0]%mod:dp[i][0];
            dp[i][1]=dp[i-1][0] + dp[i-1][1]*2;
            dp[i][1]=dp[i][1]>mod?dp[i][1]%mod:dp[i][1];
        }
        int t=sc.nextInt();
        while(t-- > 0){
            int n=sc.nextInt();
            long ans=dp[n][1]+dp[n][0];
            out.println(ans>mod?ans%mod:ans);
            out.flush();
        }
    }
}
