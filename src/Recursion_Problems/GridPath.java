...
public class GridPath {
    ...
    static long gridPath(char[][] ch,int row,int col){

        if(row<0 || col<0) return 0;
        if(ch[row][col]=='*') return 0;
        if(row==0 && col==0) return 1;
        long p1=0,p2=0;
        if(col>0 && ch[row][col-1]!='*') p1=gridPath(ch,row,col-1);
        if(row>0 && ch[row-1][col]!='*') p2=gridPath(ch,row-1,col);
        return (p1+p2)%mod;
    }
    public static void main(String[] args)throws IOException {
        ...
        out.println(gridPath(ch,n-1,n-1));
        out.close();
    }
}
