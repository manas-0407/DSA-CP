...
public class ConcertTickets {
    ...
    public static void main(String[] args)throws IOException {
        ...
        while (m-- > 0){
            int vv=sc.nextInt();
            Map.Entry<Integer,Integer> en = tm.lowerEntry(vv+1); // .lowerEntry is the catch
            if(en == null){
                out.println(-1);
            }
            else {
                out.println(en.getKey());
                if(tm.get(en.getKey()) == 1) tm.remove(en.getKey());
                else tm.put(en.getKey() , tm.get(en.getKey()) - 1);
            }
        }
        out.flush();
    }
